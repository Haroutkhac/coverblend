 ...
 * scrape-massive.ts
 *
 * Massively expands the album catalog by searching hundreds of terms on the
 * iTunes Search API (limit=200 per query), downloading new covers, computing
 * feature vectors using the identical pipeline as scrape-covers.ts, and
 * merging into the existing public/covers/index.json.
 *
 * Target: 3000+ total albums in the catalog.
 *
 * Usage: npx tsx scripts/scrape-massive.ts
 */
...
// ─────────────────────────────────────────────────────────────
// iTunes API + massive scraping logic
// ─────────────────────────────────────────────────────────────
...
// Common words that appear in album titles
const COMMON_WORDS = [
  'love', 'night', 'sun', 'moon', 'dream', 'fire', 'water', 'dark', 'light',
  'blue', 'red', 'black', 'white', 'gold', 'silver', 'heart', 'soul', 'mind',
  'life', 'death', 'time', 'space', 'star', 'sky', 'rain', 'storm', 'wind',
  'earth', 'sea', 'wave', 'rise', 'fall', 'born', 'lost', 'found', 'wild',
  'free', 'home', 'road', 'city', 'ghost', 'angel', 'demon', 'king', 'queen',
  'blood', 'bone', 'flesh', 'skin', 'eye', 'hand', 'voice', 'song', 'dance',
  'play', 'live', 'die', 'run', 'fly', 'swim', 'walk', 'talk', 'sing',
  'cry', 'laugh',
];

const ALL_QUERIES = [...SINGLE_LETTERS, ...TWO_LETTER_COMBOS, ...COMMON_WORDS];
...
async function fetchITunesAlbums(query: string): Promise<ITunesResult[]> {
  const url = `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&entity=album&limit=200&country=US`;
...
async function main() {
  console.log('=== CoverBlend MASSIVE Album Catalog Expansion ===\n');
  ...
  console.log(`  Single letters: ${SINGLE_LETTERS.length}`);
  console.log(`  Two-letter combos: ${TWO_LETTER_COMBOS.length}`);
  console.log(`  Common words: ${COMMON_WORDS.length}\n`);

  // Step 1: Fetch albums from iTunes, skipping existing ones
  console.log('Step 1: Fetching albums from iTunes (limit=200 per query)...');
  ...
  for (let i = 0; i < ALL_QUERIES.length; i++) {
    ...
    try {
      ...
      if ((i + 1) % 20 === 0 || i === ALL_QUERIES.length - 1) {
        console.log(`  [${i + 1}/${ALL_QUERIES.length}] Last: "${query}" (${results.length} results, ${newCount} new) | Total new unique: ${newAlbums.size}`);
      }
    } catch (e: any) {
      queriesFailed++;
      if ((i + 1) % 20 === 0) {
  ...
  // Step 2: Download album covers with concurrency limit of 10
  console.log('Step 2: Downloading album covers (concurrency=10)...');
  ...
  // Step 3: Analyze each downloaded image and compute feature vectors
  ...
  // Step 4: Merge into existing catalog (append, don't overwrite)
  ...
  // Step 5: Print stats and genre distribution
  console.log('=== STATS ===');
  console.log(`New albums added: ${newCatalogAlbums.length}`);
  console.log(`Total albums in catalog: ${mergedAlbums.length}`);
  console.log(`Target was 3000+: ${mergedAlbums.length >= 3000 ? 'MET' : 'NOT MET (' + mergedAlbums.length + '/3000)'}`);
  ...
  for (const [genre, count] of sortedGenres.slice(0, 20)) {
    const pct = ((count / mergedAlbums.length) * 100).toFixed(1);
