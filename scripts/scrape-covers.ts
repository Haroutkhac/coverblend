 ...
 * scrape-covers.ts
 *
 * Fetches real album covers from the iTunes Search API, downloads artwork,
 * computes feature vectors using the project's analysis pipeline, and writes
 * public/covers/index.json with real album data.
 *
...
const SEARCH_QUERIES: string[] = [
  // Top artists across genres
  'Radiohead', 'Kendrick Lamar', 'Taylor Swift', 'Daft Punk', 'Miles Davis',
  'The Beatles', 'Pink Floyd', 'Nirvana', 'Kanye West', 'Drake',
  'Frank Ocean', 'Tame Impala', 'Arctic Monkeys', 'Tyler the Creator', 'Beyonce',
  'The Weeknd', 'Billie Eilish', 'Fleetwood Mac', 'Led Zeppelin', 'David Bowie',
  'Bob Marley', 'Amy Winehouse', 'Adele', 'Mac Miller', 'Travis Scott',
  'Kid Cudi', 'MF DOOM', 'A Tribe Called Quest', 'OutKast', 'Wu-Tang Clan',
  'Eminem', 'Jay-Z', 'Nas', 'J. Cole', 'SZA',
  'Lana Del Rey', 'Bon Iver', 'Sufjan Stevens', 'Beach House', 'Aphex Twin',
  'Boards of Canada', 'Massive Attack', 'Portishead', 'Gorillaz', 'Depeche Mode',
  'Joy Division', 'The Cure', 'Talking Heads', 'Pixies', 'Bjork',
  'FKA Twigs', 'Solange', 'Blood Orange', 'Anderson .Paak', 'Thundercat',
  'Flying Lotus', 'Burial', 'James Blake', 'Four Tet', 'Caribou',
  'Bonobo', 'Tycho', 'MGMT', 'LCD Soundsystem', 'Vampire Weekend',
  'Arcade Fire', 'Neutral Milk Hotel', 'My Bloody Valentine', 'Slowdive', 'Cocteau Twins',
  'Mazzy Star', 'Elliott Smith', 'Jeff Buckley', 'Nina Simone', 'John Coltrane',
  'Thelonious Monk', 'Charlie Parker', 'Herbie Hancock', 'Weather Report', 'Pat Metheny',
  'Bill Evans', 'Chet Baker', 'Mozart', 'Beethoven', 'Bach',
  'Debussy', 'Chopin', 'Ravel',
  // Genre terms
  'hip hop classics', 'electronic music', 'indie rock', 'jazz classics',
  'classical music', 'R&B', 'punk rock', 'metal', 'ambient',
  'soul music', 'reggae', 'latin music', 'k-pop', 'afrobeats',
];
...
async function main() {
  console.log('=== CoverBlend Album Scraper ===\n');
  ...
  // Step 1 & 2: Fetch albums from iTunes and deduplicate
  console.log(`Step 1: Fetching albums from iTunes (${SEARCH_QUERIES.length} queries)...`);
  const allAlbums = new Map<number, ITunesResult>();

  for (let i = 0; i < SEARCH_QUERIES.length; i++) {
    const query = SEARCH_QUERIES[i];
    process.stdout.write(`  [${i + 1}/${SEARCH_QUERIES.length}] Searching "${query}"...`);
    try {
      const results = await fetchITunesAlbums(query);
      let newCount = 0;
      for (const album of results) {
        if (!allAlbums.has(album.collectionId)) {
          allAlbums.set(album.collectionId, album);
          newCount++;
        }
      }
      console.log(` ${results.length} results, ${newCount} new (total: ${allAlbums.size})`);
    } catch (e: any) {
  ...
  console.log(`\nTotal unique albums found: ${allAlbums.size}\n`);

  if (allAlbums.size === 0) {
    console.error('No albums found. Exiting.');
    process.exit(1);
  }

  // Step 3: Download album covers with concurrency limit of 5
  console.log('Step 3: Downloading album covers...');
  const albumList = Array.from(allAlbums.values());
  let downloaded = 0;
  ...
  // Step 4: Analyze each downloaded image
  console.log('Step 4: Analyzing images and computing feature vectors...');
  const catalogAlbums: AlbumEntry[] = [];
  let analyzed = 0;
  ...
  console.log('Step 5: Writing catalog...');
  const catalog = {
    version: 2,
  ...
  console.log(`  Wrote ${catalogPath} with ${catalogAlbums.length} albums\n`);

  // Step 6: Delete old SVG covers
  console.log('Step 6: Deleting old SVG covers...');
  let deletedCount = 0;
  for (let i = 1; i <= 150; i++) {
    const svgPath = path.join(COVERS_DIR, `cover-${String(i).padStart(3, '0')}.svg`);
    if (fs.existsSync(svgPath)) {
      fs.unlinkSync(svgPath);
      deletedCount++;
    }
  }
  console.log(`  Deleted ${deletedCount} SVG files\n`);

  // Summary
  console.log('=== DONE ===');
  console.log(`Total albums in catalog: ${catalogAlbums.length}`);
  console.log(`Target was 200+: ${catalogAlbums.length >= 200 ? 'MET' : 'NOT MET'}`);
}
