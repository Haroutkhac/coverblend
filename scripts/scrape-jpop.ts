 ...
 * scrape-jpop.ts
 *
 * Expands the album catalog with J-Pop and Japanese music by searching
 * artist names and genre terms on the iTunes Search API using the Japanese
 * store (country=JP, limit=200), downloading 300x300 covers, computing
 * feature vectors using the identical pipeline as scrape-covers.ts, and
 * merging into the existing public/covers/index.json.
 *
 * Usage: npx tsx scripts/scrape-jpop.ts
 */
...
// J-Pop artist queries
const ARTIST_QUERIES: string[] = [
  'YOASOBI', 'Ado', 'Kenshi Yonezu', 'Fujii Kaze', 'RADWIMPS',
  'Official HIGE DANdism', 'Mrs. GREEN APPLE', 'King Gnu', 'Vaundy', 'Aimer',
  'LiSA', 'Eve', 'Yorushika', 'Aimyon', 'back number',
  'Hikaru Utada', 'Sheena Ringo', 'Cornelius', 'Perfume', 'Capsule',
  'Kyary Pamyu Pamyu', 'Daoko', 'Nujabes', 'Toe', 'MONO band',
  'Boris band', 'Melt-Banana', 'Fishmans', 'Number Girl', 'Supercar band',
  'Asian Kung-Fu Generation', 'BUMP OF CHICKEN', "L'Arc-en-Ciel", 'X Japan', 'Dir En Grey',
  'Luna Sea', 'GLAY', 'Mr. Children', 'Southern All Stars', 'Spitz band',
  "B'z", 'Dreams Come True', 'ZARD', 'Yumi Matsutoya', 'Tatsuro Yamashita',
  'Mariya Takeuchi', 'Taeko Onuki', 'Haruomi Hosono', 'Yellow Magic Orchestra', 'Ryuichi Sakamoto',
  'Susumu Hirasawa', 'Ichiko Aoba', 'tricot band', 'CHAI band japan', 'Wednesday Campanella',
  'Gesu no Kiwami Otome', 'Sekai no Owari', 'ONE OK ROCK', 'Scandal band japan', 'BABYMETAL',
  'Band-Maid', 'Polkadot Stingray', 'Frederic band', 'Sakanaction', 'Suchmos',
  'Nulbarich', 'cero band', 'Lamp band japan', 'Shinsei Kamattechan', 'the pillows',
  'Shonen Knife', 'Pizzicato Five', 'Fantastic Plastic Machine', 'Mondo Grosso', 'DJ Krush',
  'Ken Ishii', 'Towa Tei', 'Denki Groove', 'Cornelius', 'Takako Minekawa',
];

// J-Pop genre queries
const GENRE_QUERIES: string[] = [
  'jpop', 'j-pop', 'city pop', 'japanese rock', 'visual kei',
  'anime soundtrack', 'japanese electronic', 'shibuya kei', 'japanese indie', 'japanese hip hop',
  'vocaloid', 'japanese jazz', 'enka', 'japanese ambient', 'japanese math rock',
];
...
async function fetchITunesAlbums(query: string): Promise<ITunesResult[]> {
  const url = `https://itunes.apple.com/search?term=${encodeURIComponent(query)}&entity=album&limit=200&country=JP`;
  ...
  // Handle 429 rate limiting: wait 5s and retry once
  if (response.status === 429) {
    console.warn(`  Rate limited (429) for "${query}", waiting 5s and retrying...`);
    await sleep(5000);
    try {
      response = await fetch(url);
    } catch {
    ...
    if (response.status === 429) {
      console.warn(`  Still rate limited for "${query}", skipping.`);
      return [];
    }
  }
  ...
  try {
    ...
    console.warn(`  JSON parse error for "${query}": ${e.message}`);
    return [];
  }
}
...
async function main() {
  console.log('=== CoverBlend J-Pop Album Scraper ===\n');
  ...
  // Step 1: Load existing catalog to get existing IDs
  const catalogPath = path.join(COVERS_DIR, 'index.json');
  let existingAlbums: AlbumEntry[] = [];
  let existingCollectionIds = new Set<number>();

  if (fs.existsSync(catalogPath)) {
    const existingCatalog = JSON.parse(fs.readFileSync(catalogPath, 'utf-8'));
    existingAlbums = existingCatalog.albums || [];
    existingCollectionIds = new Set<number>(
  ...
  console.log(`  Artist queries: ${ARTIST_QUERIES.length}`);
  console.log(`  Genre queries: ${GENRE_QUERIES.length}\n`);

  // Step 2: Fetch albums from iTunes JP store, skipping existing ones
  console.log('Step 2: Fetching albums from iTunes JP store (limit=200 per query, country=JP)...');
  ...
  for (let i = 0; i < ALL_QUERIES.length; i++) {
    ...
    try {
      ...
      for (const album of results) {
        // Skip if already in existing catalog or already found this run
      ...
      queriesCompleted++;
      console.log(`  [${i + 1}/${ALL_QUERIES.length}] "${query}" => ${results.length} results, ${newCount} new (total new unique: ${newAlbums.size})`);
    } catch (e: any) {
      queriesFailed++;
      console.log(`  [${i + 1}/${ALL_QUERIES.length}] "${query}" ERROR: ${e.message}`);
    }
  ...
  console.log(`\nQueries completed: ${queriesCompleted}, failed: ${queriesFailed}`);
  console.log(`Total new unique albums found: ${newAlbums.size}\n`);
  ...
  // Step 3: Download 300x300 album covers with concurrency limit of 10
  console.log('Step 3: Downloading 300x300 album covers (concurrency=10)...');
  ...
  const downloadResults = await runWithConcurrency(downloadTasks, 10);
  console.log(`  Downloads complete: ${downloaded} succeeded (${downloadSkipped} already existed), ${downloadFailed} failed\n`);

  // Step 4: Analyze each downloaded image and compute feature vectors
  console.log('Step 4: Analyzing images and computing feature vectors...');
  ...
  // Step 5: Merge into existing catalog (append, don't overwrite)
  console.log('Step 5: Merging into existing catalog...');
  ...
  console.log(`  Added: ${newCatalogAlbums.length} new J-Pop albums`);
  console.log(`  Total: ${mergedAlbums.length} albums\n`);

  // Step 6: Report stats
  console.log('=== J-POP SCRAPE STATS ===');
  console.log(`Queries run: ${queriesCompleted}/${ALL_QUERIES.length}`);
  console.log(`New unique albums discovered: ${newAlbums.size}`);
  console.log(`Downloaded: ${downloaded} (${downloadSkipped} already existed)`);
  console.log(`Download failures: ${downloadFailed}`);
  console.log(`Analyzed successfully: ${analyzed}`);
  console.log(`Analysis failures: ${analysisFailed}`);
  console.log(`New albums added to catalog: ${newCatalogAlbums.length}`);
  console.log(`Total albums in catalog: ${mergedAlbums.length}\n`);

  // Genre distribution of new albums
  const genreCounts = new Map<string, number>();
  for (const album of newCatalogAlbums) {
    const genre = album.genre || 'Unknown';
    genreCounts.set(genre, (genreCounts.get(genre) || 0) + 1);
  }
  const sortedGenres = Array.from(genreCounts.entries()).sort((a, b) => b[1] - a[1]);
  console.log('Genre distribution of new J-Pop albums:');
  for (const [genre, count] of sortedGenres) {
    const pct = ((count / newCatalogAlbums.length) * 100).toFixed(1);
    console.log(`  ${genre}: ${count} (${pct}%)`);
  }
  ...
  console.log('\nTop 20 artists in new J-Pop albums:');
